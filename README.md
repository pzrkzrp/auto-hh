...
npm run resume:show -- pavel             # показать содержимое resumes/pavel.md
npm run resume:register -- pavel         # зарегистрировать resumes/pavel.md
```
...
npx auto-hh resume show pavel
npx auto-hh resume register pavel
```
...
| `-j, --job <id>` | ID документа `search_jobs` для обновления статуса (веб-бэкенд) |
| `-u, --user <id>` | userId для мультиарендности (веб-бэкенд) |
...
| `-q, --queue` | Читать вакансии из `apply_queue` (веб-бэкенд) вместо дайджеста |
| `-u, --user <id>` | userId для мультиарендности (веб-бэкенд) |
